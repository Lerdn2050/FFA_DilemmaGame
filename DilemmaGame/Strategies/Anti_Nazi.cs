...
namespace DilemmaGame.Strategies
{
    class Anti_Nazi
    {
        ...
        {
            return "Предаёт слишком предвзятых  ";
        }
        ...
        public static bool Strategy(Game.GameInfo GameInfo, int MyNumber, int OpponentNumber)
        {
            ...
            for (int i = MyGI.Rounds.Length - 1; i >= 0; i--)
            {
                if (MyGI.Rounds[i].Player1_Choice)
                {
                    ...
                    {
                        ratio[MyGI.Rounds[i].Player2] = coops[MyGI.Rounds[i].Player2] > betrays[MyGI.Rounds[i].Player2] ? coops[MyGI.Rounds[i].Player2] / betrays[MyGI.Rounds[i].Player2] : betrays[MyGI.Rounds[i].Player2] / coops[MyGI.Rounds[i].Player2];
                    }
                    else
                    {
                        ratio[MyGI.Rounds[i].Player2] = coops[MyGI.Rounds[i].Player2];
                    }
                }
                else
                {
                    ...
                    {
                        ratio[MyGI.Rounds[i].Player2] = betrays[MyGI.Rounds[i].Player2];
                    }
                }
            }
